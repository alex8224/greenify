...
from gtornado.green import Waiter, Watcher
...
cdef int wait_gtornado_event(greenify_watcher* watchers, int nwatchers, int timeout_in_ms) with gil:
    cdef int fd, event
...
def greenify():
    greenify_set_wait_callback(wait_gtornado_event)
